@extends('_layout.default') @section('intoheader') {{ HTML::style('css/jquery.dataTables.min.css') }} {{ HTML::script('js/jquery.dataTables.min.js') }}
<script type="text/javascript">
    $(document).ready(function() {
        $('#example').DataTable();
    });
</script>
@stop @section('header') {{ link_to_route('dashboard', 'Your Profile'). " | " }} @if(Auth::check()) {{ link_to_route('logout', 'Logout') }} @else {{ link_to_route('user.create', 'Register') }} {{ link_to_route('login', 'Login') }} @endif @stop @section('content')
